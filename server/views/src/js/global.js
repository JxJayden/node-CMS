(function () {
    var global = {
        bindEvent: function () {
            ...
            $('body').on('click', '.card-star', function () {
            ...
            $('body').on('click', '.choose-card-addcombo', function () {
            ...
            $('.select-language').click(function (e) {
                ...
                if ($(this).hasClass('select-language-acitve')) {
                    return false;
                }
                if ($(this).data('language') === 'cn') {
                    if (~url.indexOf('?language=en')) {
                        url = window.location.search === '?language=en' ?
                            url.replace('?language=en', '') :
                            url.replace('?language=en', '?');
                    } else {
        ...
        ProductToQuote: function (productId, state) {
            var _this = this;
            var localProduct = store.get('quote').product;
            if (localProduct) {
                if (state === 'add') {
                    if (~localProduct.indexOf(productId)) {
                        return;
                    } else {
                        localProduct.push(productId);
                        _this.setQuoteStage('product', localProduct);
                        _this.updateProductTemplate(productId, state);
                    }
                } else if (state === 'del') {
                    if (~localProduct.indexOf(productId)) {
                        localProduct.splice(localProduct.indexOf(productId), 1);
                        _this.setQuoteStage('product', localProduct);
                        _this.updateProductTemplate(productId, state);
                    } else {
                        return;
        ...
        comboToQuote: function (combo, state) {
            var _this = this;
            var localCombo = store.get('quote').combo;
            if (localCombo) {
                if (state === 'add') {
                    var isAdd = false;
                    for (var i = 0; i < localCombo.length; i++) {
                        if (localCombo[i].combo === combo.combo) {
                            isAdd = true;
                        }
                    }
                    if (!isAdd) {
                        localCombo.push(combo);
                        _this.setQuoteStage('combo', localCombo);
        ...
        removeAll: function () {
            ...
            this.setQuoteStage('product', []);
            this.setQuoteStage('combo', []);
            this.delStar();
        },
        // 初始化列表
        setQuoteTemplate: function () {
            ...
            if (quoteLocalStage.product.length > 0) {
                $.post('/api/product/arr', {
                    arr: JSON.stringify(quoteLocalStage.product)
                }, function (data) {
                    var templateData = data;
                    templateData.chinese = ~window.location.search.indexOf('language=en') ? false : true;
                    var html = template('nav-quote-product', templateData);
        ...
        // 弹窗提示
        showDialog: function (tips) {
            var _this = this;
            if ($('.dialog').length <= 0) {
                $('body').append([' <div class="dialog">',
                    '<div class="dialog-overlay"></div>',
                    '<div class="dialog-alert">',
                    '<div class="dialog-img"><img width="50%" src="//ohnpdrepj.bkt.clouddn.com/star-succeed.png"></div>',
                    '<div>',
                    '收藏商品成功！',
                    '</div>',
        ...
        setQuoteStage: function (updateKey, updateValue) {
            var quoteLocalStage = store.get('quote'),
                _this = this,
                lastTime = quoteLocalStage ? (quoteLocalStage.updateTime || quoteLocalStage.createTime) : new Date();

            if (quoteLocalStage && quoteLocalStage.createTime) {
                if (updateValue && updateKey) {
                    try {
                        store.transact('quote', function (value) {
                ...
                } else if (!quoteLocalStage.localStorageNew) {
                    _this.setQuoteStage('localStorageNew', true);
                }
            } else {
                store.remove('quote');
                /*
